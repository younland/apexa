...
async function registerOfflineIcons() {
  if (!addCollection(lucideIcons)) {
    console.warn('load offline lucide icons err')
  }
}
    ...
    try {
      ...
      if (iconName) {
        ...
        if (parsedSVG.attribs?.viewBox) {
          const viewBox = parsedSVG.attribs.viewBox.split(' ') as any
          if (viewBox.length === 4) {
